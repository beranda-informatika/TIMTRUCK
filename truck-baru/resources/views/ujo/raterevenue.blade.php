        ...
        @foreach ($detailrate as $data)
            <tr @if ($data->f_edit == '0') style="background:rgb(239, 237, 237); "  readonly @endif>
                ...
                <td style="text-align: right;">
                    @if ($data->rateid == '10011')
                        <input class="innilai" type="text" name="nominal[]" id="nominal{{ $i }}"
                            value="{{ $data->nominal }}" style="text-align: right; width: 75px; background:rgb(239, 237, 237); " require readonly>

                    @else
                        <input class="innilai" type="text" name="nominal[]" id="nominal{{ $i }}"
                            value="{{ $data->nominal }}"
                            @if ($data->f_edit == '0') style="width: 75px; background:rgb(239, 237, 237);  text-align:right" require readonly
                        @else

                        style="text-align: right; width: 75px" @endif
                            @if ($data->f_invoice == '1') style="width: 75px; background:rgb(3, 120, 62);  text-align:right" require readonly
                        @else
                ...
                <td style="text-align: right;">
                    <input class="innilai" type="number" name="qty[]" id="qty{{ $i }}"
                        value="{{ $data->qty }}"
                        @if ($data->f_edit == '0') style="text-align: left;  background:rgb(239, 237, 237);  text-align:right" require readonly
                        @else

                        style="text-align: right; " @endif
                        @if ($data->f_invoice == '1') style="text-align: left;  background:rgb(131, 35, 35);  text-align:right" require readonly
                        @else
...
<div style="text-align: right; padding:10px">
    <div class="btn btn-sm btn-info  btn-action"
        data-url="{{ route('shipment.inraterevenue', $shipment->shipmentid) }}" id="btnAction1">Add Rate</div>
