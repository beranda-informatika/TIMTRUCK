<form role="form" class="parsley-examples" action="{{ route('shipment.store') }}" method="POST">
    ...
    <div class="mb-3">
        ...
        <input type="text" id="kdkategori" name="kdkategori" class="form-control"
            value="{{ $quotation->getgroupquotation->kdkategori }}" readonly style="background: rgb(235, 234, 234)">
    ...
    <div class="mb-3">
        ...
        <input type="text" id="typeroute" name="typeroute" class="form-control" value="{{ $quotation->typeroute }}"
            readonly style="background: rgb(235, 234, 234)">
    </div>
    <div class="mb-3">
    ...
    <div class="mb-3">
        ...
        <div data-url="{{ route('quotation.forminsales') }}" class="btn btn-sm btn-info  btn-action">+ New Sales if
            not
            available</div>
            ...
            <div class="controls">
                <div class="form-group">
                    <div class="entry input-group">
                        <input class="form-control" name="locationdrop[]" type="text" placeholder="Drop Location"
...
<script>
    ...
    $(function() {
        $(document).on('click', '.btn-addpickup', function(e) {
            e.preventDefault();

            var controlForm = $('.controlspickup:first'),
                currentEntry = $(this).parents('.entry:first'),
                newEntry = $(currentEntry.clone()).appendTo(controlForm);

            newEntry.find('input').val('');
            controlForm.find('.entry:not(:last) .btn-addpickup')
                .removeClass('btn-addpickup').addClass('btn-remove')
                .removeClass('btn-success').addClass('btn-danger')
                .html('remove');
        }).on('click', '.btn-removepickup', function(e) {
            $(this).parents('.entry:first').remove();
